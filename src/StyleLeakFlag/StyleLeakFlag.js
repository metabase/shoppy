...
export const StyleLeakFlag = () => {
  ...
  const isFontLeaking =
    currentFontFamily !== "IBM Plex Sans" || currentFontSize !== "16px";
  const pillClasses = isFontLeaking ? "tw-bg-red-400" : "tw-bg-green-400";
  const pillText = isFontLeaking
    ? "Fonts are leaking 🫠"
    : "Fonts aren't leaking 😎";

  return (
    <>
      <div
        className={`tw-text-white tw-font-bold tw-grid tw-place-items-center tw-px-2 tw-py-1 tw-rounded ${pillClasses}`}
        ref={divRef}
      >
        {pillText}
      </div>
