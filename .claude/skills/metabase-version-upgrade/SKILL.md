...
### Step 0: Upgrade Plan Checklist (required before any other work)
...
### Step 0.1: Parallel Plan (required)

Write a brief plan that explicitly states:
- "I will run Step 1 locally while simultaneously launching Step 2 (d.ts extraction + upgrade guide) in parallel."

### Step 0.2: Path Selection
...
### Step 1: Scan the project code (NO sub-agent)
...
```bash
TMPDIR=$(mktemp -d)
mkdir -p "$TMPDIR/current" "$TMPDIR/target"
(cd "$TMPDIR/current" && npm pack @metabase/embedding-sdk-react@{CURRENT} --quiet 2>/dev/null && tar xzf *.tgz)
(cd "$TMPDIR/target" && npm pack @metabase/embedding-sdk-react@{TARGET} --quiet 2>/dev/null && tar xzf *.tgz)
echo "TMPDIR=$TMPDIR"
```

Replace `{CURRENT}` and `{TARGET}` with the actual version numbers.

Then check if `$TMPDIR/current/package/dist/index.d.ts` and `$TMPDIR/target/package/dist/index.d.ts` both exist.

- **If BOTH exist** → primary path confirmed. Continue to Step 2b.
- **If EITHER is missing** → switch to **fallback path** (Step 2-fallback below). Output: "d.ts not available for version X.Y.Z, switching to fallback (docs fetch)."

#### Step 2b: Diff the d.ts files

Run:
```bash
diff -u "$TMPDIR/current/package/dist/index.d.ts" "$TMPDIR/target/package/dist/index.d.ts" || true
```

Save the diff output — this is the source of truth for all API changes.

#### Step 2c: Fetch upgrade guide (runs in parallel with 2a/2b via sub-agent)

Launch ONE sub-agent to WebFetch:
...
### Step 2-fallback: Fetch docs via sub-agents (fallback path)

If the primary path is not available (d.ts missing for either version, OR EmbedJS/Modular Embedding upgrade):
...
- Filter by keywords + best-effort symbol list (StaticQuestion, InteractiveQuestion, InteractiveDashboard, EditableDashboard, CreateDashboardModal, CollectionBrowser, MetabaseProvider, MetabaseAuthConfig, MetabaseTheme, defineMetabaseAuthConfig, plus what you already know from Step 1 imports if available).
...
- If errors:
    ...
    - Apply fixes
    - Re-run
      Repeat until zero SDK-related errors.
...
2) Deprecations
3) Notes (0.58+ architecture, instance version requirement)
4) Path used (primary d.ts diff / fallback docs)

## Anti-skip enforcement for parallelism (hard)

When starting execution:
- You MUST perform Step 1 Grep + Reads AND launch Step 2 (d.ts extraction OR fallback sub-agents + upgrade guide) in the same overall tool-call message.
- If you fail to do so, you MUST stop and restart execution with the correct parallel launch.
